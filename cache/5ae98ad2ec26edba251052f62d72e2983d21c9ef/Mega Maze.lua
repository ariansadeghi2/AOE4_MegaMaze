...
function carveMaze(x, y)
    ...
    terrainLayoutResult[x][y].terrainType = tt_plains_smooth
    visited[x][y] = true

    -- Shuffle directions to ensure randomized maze paths
    local directions = shuffle({{1, 0}, {0, 1}, {-1, 0}, {0, -1}})

    for i, dir in ipairs(directions) do
        local nx, ny = x + dir[1] * 2, y + dir[2] * 2

        -- Check if the new cell is within bounds and has not been visited
        if nx > 0 and nx <= gridSize and ny > 0 and ny <= gridSize and not visited[nx][ny] then
...
startX, startY = math.ceil(gridSize / 2), math.ceil(gridSize / 2)
